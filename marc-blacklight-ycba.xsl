...
<xsl:stylesheet exclude-result-prefixes="xs" version="1.0"
    ...
    <xsl:template name="collection">
        <xsl:param name="set"/>
            ...
            <xsl:when test="$set='smlmap' or $set='smlmapl'">
                <xsl:element name="collection_t">
                    <xsl:text>Sterling Memorial Library Map Collection</xsl:text>
                </xsl:element>
            </xsl:when>
    ...
    <xsl:template name="language_note">
        <xsl:for-each select="marc:datafield[@tag='546']">
            <xsl:element name="language_note_t">
    ...
    <xsl:template name="resourceURL">
        <xsl:for-each select="marc:datafield[@tag='856']">
            <xsl:element name="resourceURL_t">
                <xsl:element name="caption_t">
                    <xsl:choose>
    ...
    <xsl:template name="callnumber">
    	<xsl:variable name="bibID">
           ...
           <xsl:variable name="fullURL">
                <xsl:value-of select="concat('http://columbus.library.yale.edu:8055/OAI_BAC/src/OAIOrbisTool.jsp?verb=GetRecord&amp;identifier=oai:orbis.library.yale.edu:',$bibID,'&amp;metadataPrefix=oai_dc')" />
        </xsl:variable>
        <xsl:choose>
    ...
    <xsl:template name="author_additional">
        <xsl:for-each
            ...
            <xsl:element name="author_additional_t">
                <xsl:for-each select="marc:subfield">
                    <xsl:value-of select="normalize-space(.)"/>
                    <xsl:if test="following-sibling::marc:subfield">
                        <xsl:text> </xsl:text>
